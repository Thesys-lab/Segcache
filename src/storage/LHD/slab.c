...
struct slabclass slabclasses[SLABCLASS_MAX_ID + 1]; /* collection of slabs bucketed by slabclass */
...
pthread_mutex_t item_lock[1u<<16u];
extern pthread_mutex_t temp_lock;
extern uint64_t LHD_num_reconfig;
extern int ageCoarseningShift;
//extern struct item     tags[100000000];
 ...
 */
struct item *
slab_to_item(struct slab *slab, uint32_t idx, size_t size)
{
    struct item *it;
...
#if defined(CC_ASSERT_PANIC) || defined(CC_ASSERT_PANIC)
...
slab_id(size_t size)
{
    ...
    while (imax >= imin) {
        id = (imin + imax) / 2;
        if (size > slabclasses[id].size) {
            imin = id + 1;
        } else if (id > SLABCLASS_MIN_ID && size <= slabclasses[id - 1].size) {
            imax = id - 1;
        } else {
            break;
        }
...
_slab_heapinfo_setup(void)
{
    ...
    pthread_mutex_init(&heapinfo.mtx, NULL);
    heapinfo.base = NULL;
    if (prealloc) {
    ...
    heapinfo.slab_locked = cc_zalloc((sizeof(int) * heapinfo.max_nslab));
...
slab_setup(slab_options_st *options, slab_metrics_st *metrics)
{
    ...
    if (options != NULL) {
        ...
        slab_mem = option_uint(&options->heap_mem);
        prealloc = option_bool(&options->prealloc);
        evict_opt = option_uint(&options->evict_opt);
        ...
        use_cas = option_bool(&options->use_cas);
        hash_power = option_uint(&options->hash_power);
        slab_datapool = option_str(&options->datapool_path);
        slab_datapool_name = option_str(&options->datapool_name);
        prefault = option_bool(&options->datapool_prefault);
    }
...
_slab_evict_one(struct slab *slab)
{
    ...
    for (i = 0; i < p->nslabs; i++) {
        if (p->slab_list[i] == slab) {
            p->slab_list[i] = p->slab_list[--p->nslabs];
        }
    }
    ...
    for (i = 0; i < p->nitem; i++) {
        it = slab_to_item(slab, i, p->size);
        while ((!__atomic_compare_exchange_n(
                &it->locked, &unlock, 1, false, __ATOMIC_RELEASE, __ATOMIC_RELAXED))) {
            ;
        }

        while (__atomic_load_n(&it->refcount, __ATOMIC_RELAXED) != 0) {
            ;
        }

        log_debug("slab move evict %.*s", it->klen, item_key(it));
...
{
    log_debug("init slab %d %p", id, slab);
    ...
    memset(slab->data, 0, slab_size-SLAB_HDR_SIZE);
...
/*
 * this function is called only when the given slabclass has no slab
 *
 *
 *
 *
...
_slab_get(uint8_t id)
{
    ...
    pthread_mutex_lock(&heapinfo.mtx);

    if (slabclasses[id].nslabs != 0) {
        pthread_mutex_unlock(&heapinfo.mtx);
        return CC_OK;
    }

    ASSERT(slabclasses[id].next_item_in_slab == NULL);
    ASSERT(SLIST_EMPTY(&slabclasses[id].free_itemq));
...
//    ASSERT(pthread_mutex_trylock(&slabclasses[id].lock) != 0);
    ...
    log_verb("get free q it %p at offset %"PRIu32" with id %"PRIu8 " %d left first %p", it,
            it->offset, it->id, p->nfree_itemq, SLIST_FIRST(&p->free_itemq));
