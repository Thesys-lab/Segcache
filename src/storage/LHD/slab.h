...
#define SLAB_OPTION(ACTION)                                                                                   \
    ...
    ACTION( heap_mem,               OPTION_TYPE_UINT,   SLAB_MEM,            "Max memory by slabs (byte)"    )\
    ACTION( prealloc,               OPTION_TYPE_BOOL,   SLAB_PREALLOC,       "Pre-allocate slabs at setup"   )\
    ACTION( evict_opt,              OPTION_TYPE_UINT,   SLAB_EVICT_OPT,      "Eviction strategy"             )\
    ...
    ACTION( use_cas,                OPTION_TYPE_BOOL,   SLAB_USE_CAS,        "Store CAS value in item"       )\
    ACTION( hash_power,             OPTION_TYPE_UINT,   HASH_POWER,          "Power for lookup hash table"   )\
    ACTION( datapool_path,          OPTION_TYPE_STR,    SLAB_DATAPOOL,       "Path to data pool"             )\
    ACTION( datapool_name,          OPTION_TYPE_STR,    SLAB_DATAPOOL_NAME,  "Slab data pool name"           )\
    ACTION( datapool_prefault,      OPTION_TYPE_BOOL,   SLAB_PREFAULT,       "Prefault data pool"            )
...
//    int32_t          refcount;     /* # items that can't be evicted */
#ifdef USE_LHD
    double            rank;
#endif
