...
struct item *
hashtable_rand(struct hash_table *ht)
{
    uint64_t idx;
    ...
    while (it == NULL) {
        idx = prand() & HASHMASK(ht->hash_power);
        bucket = &(ht->table[idx]);
        it = SLIST_FIRST(bucket);
    }

    return it;
}

uint32_t get_hv(const char *key, size_t klen) {
    uint32_t hv;
