...
bench_storage_set(struct benchmark_entry *e)
{
    ...
    item_rstatus_e status = item_reserve_with_ttl(&it, &key, &val, val.len, 0, e->ttl);
    if (status != ITEM_OK)
        return CC_ENOMEM;
...
bench_storage_cas(struct benchmark_entry *e)
{
    ...
    oit = item_get(&key, &old_cas);
    if (oit == NULL){
        return CC_ERROR;
    }

    /* we need to release this before reserve, otherwise, it may have a deadlock
     * an alternative is to avoid choosing segment with non-zero r_refcnt */
    item_release(oit);

    struct bstring val = {.data=e->val, .len=e->val_len};
    item_rstatus_e status = item_reserve_with_ttl(&nit, &key, &val, val.len, 0, e->ttl);
