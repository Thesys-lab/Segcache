...
static void warm_up(struct benchmark *b, bool val_num, bool non_overlap) {
    ...
    if (non_overlap)
        key_start = n_entries * 2;

    for (uint32_t key = key_start; key <= (uint32_t) n_entries; key++) {
