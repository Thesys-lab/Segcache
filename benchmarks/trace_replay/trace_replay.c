...
volatile int64_t         global_op_cnt[op_invalid];
volatile static uint64_t global_n_req     = 0;
volatile static uint64_t global_n_get_req = 0;
volatile static uint64_t global_n_miss    = 0;
...
_trace_replay_thread(void *arg)
{
    ...
    static __thread uint64_t n_get_req          = 0;
    static __thread uint64_t n_req              = 0;
    static __thread uint64_t op_cnt[op_invalid] = {0};

    uint64_t idx = (uint64_t) arg;

    char thread_name[12];
    snprintf(thread_name, 12, "thread%"PRIu64, idx);
...
//        }
        ...
        if (time_proc_sec() > last_check_ts && time_proc_sec() % 200 == 0) {
//            verify_objstore();
//            verify_hashtable();
            last_check_ts = time_proc_sec();
        }
    }

    __atomic_add_fetch(&global_n_req, n_req, __ATOMIC_RELAXED);
    __atomic_add_fetch(&global_n_get_req, n_get_req, __ATOMIC_RELAXED);
    __atomic_add_fetch(&global_n_miss, n_miss, __ATOMIC_RELAXED);
    for (int i = 0; i < op_invalid; i++) {
        __atomic_add_fetch(&global_op_cnt[i], op_cnt[i], __ATOMIC_RELAXED);
    }
...
main(int argc, char *argv[])
{
    ...
    printf("%s total benchmark runtime: %.2lf s, throughput %.2lf M QPS\n",
        argv[1], duration_sec(&d), global_n_req / duration_sec(&d) / 1000000);
    printf("average operation latency: %.2lf ns, miss ratio %.4lf\n",
        duration_ns(&d) / global_n_req, (double) global_n_miss / global_n_get_req);

    for (op_e op = op_get; op < op_invalid; op++) {
        if (global_op_cnt[op] == 0) {
            continue;
        }
        printf("op %16s %16"PRIu64 "(%.4lf)\n", op_names[op], global_op_cnt[op],
            (double) global_op_cnt[op] / global_n_req);
    }
